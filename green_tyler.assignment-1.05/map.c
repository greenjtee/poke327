...
// ncurses print map, does not refresh screen
void print_map_nc(map_t *m) {
  ...
  for (i = 0; i < world.num_trainers; i++) {
    ...
    mvaddch(m->trainers[i].pos[dim_y]+WINDOW_MAIN_TOP, m->trainers[i].pos[dim_x], c);
    // printf("---------------------------\n");
  }

  mvaddch(world.pc.pos[dim_y]+WINDOW_MAIN_TOP, world.pc.pos[dim_x], '@');
